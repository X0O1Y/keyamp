...
#+begin_src bash
  mosh-server new -p 66566:66569 -s -c 256 -l LANG=en_US.UTF-8 -- bash -c "if [ $(ps -C mosh-server -o pid= | wc -l) -eq 1 ]; then kill $(ps --no-headers --sort=start_time -C mosh-server -o pid | tail -n 1); fi && tmux -u -f ~/.tmux-emacs.conf -L emacs new -As emacs 'emacs'; exec bash -l"
#+end_src
